...
char *create_md5_code(const char *data_type, const char *data)
{
    char            input_str_p[1024];
    char            *md5_value_p = NULL;
    unsigned int    encode_file_flag = 0;

	if (0 == strcmp("file", data_type) || 0 == strcmp("File", data_type) || 0 == strcmp("FILE", data_type))
	{
		encode_file_flag = 1;
	}
	...
	if((NULL == data) || (0 == strlen(data))){
		...
		strcpy(input_str_p, data);		
	}
	
	if(encode_file_flag){
		if (0 == access(input_str_p, F_OK)) {
